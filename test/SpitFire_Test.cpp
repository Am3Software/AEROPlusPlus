...
#include "Plotter3D.h"
#include "WeightsAndBlanceExcelWriter.h"
...
int main()
{
    ...
    ac.fus.id = fus.id;
    ac.fus.length = fus.length;
    ac.fus.diameter = fus.diameter;
    ac.fus.width = fus.width;
    ...
    settings.X_cg = wing.xloc + wing.deltaXtoLEMAC + 0.25 * settings.Cref;                                                   // Center of gravity
    settings.Y_cg = 0.0;
    settings.Z_cg = 0.25 * fus.diameter;
    settings.Mach = 0.78;                                                // Mach number
...
double WTOold = builder.getCommonData().getWTO();
double WTOnew = 0.0;
double fuelMass = builder.getCommonData().getFuelWeight();

// CD0Calculator cd0Calc(aircraftName, builder, ac, settings, wing, horizontal, vertical, fus, nac);

// cd0Calc.calculateCD0Wing();
// cd0Calc.calculateCD0HorizontalTail();
// cd0Calc.calculateCD0VerticalTail();
// cd0Calc.calculateCD0FusiFormBody(fus.id);
// cd0Calc.calculateCD0FusiFormBody(nac.id);
...
for (double AoA = aircrfatalphaZeroLiftAngle; AoA <= 10; AoA += 1.0)
{
  ...
  Plot plotEfficinency(liftCoefficients, efficiencyAircraft,
                          ...
                          "L/D (-)",
                          "Efficiency",
                          "L/D, M = 0.78, Alt = 35000 ft",
                          "lines", "1", "1", "", "", "orange");

// std::cout << "\n=== DRAG COEFFICIENT CALCULATION ===" << std::endl;
// // std::cout << "CD0 Total Aircraft: " << CD0TotalAircraft << std::endl;
// std::cout << "CD Total Aircraft (with drag increment): " << dragCoefficentAircraft << std::endl;

// COG::Weights weightData;
// COG::COGDATA cogData;
...
// std::vector<double> WTOhistory; // Per tracciare l'evoluzione di WTO
// std::vector<int> iterationHistory; // Per tracciare il numero di iterazione

// while (std::abs(1.0 - WTOnew / WTOold) > 0.01 && iteration < maxIterations)
// {

    
//     // ── Aggiorna WTOold ──────────────────────────────────────────────────────
//     if (iteration == 0){
//         WTOold = builder.getCommonData().getWTO();  // valore iniziale di tentativo
//         data.commonData.setFuelWeight(fuelMass);
//         WTOhistory.push_back(WTOold);
//         iterationHistory.push_back(iteration);
//     }

//     else {

//         WTOold = WTOnew;                            // WTO convergente dal giro prima
       

//     }

//     data.commonData.setWTO(WTOold);

//       // ── Calcolo carburante ───────────────────────────────────────────────────
//     if (iteration > 0)
//     {
//         FuelCalculatorBreguet fuelCalc;
//         fuelMass = fuelCalc.calculateFuelMassJetDriven(
//                         builder.getEngineData().getAircraftEngineType(),
//                         WTOold,
//                         builder.getCommonData().getRangeCovered(),
//                         builder.getEngineData().getSFCJ(),
//                         builder.getEngineData().getThrustLbf(),
//                         0.85, settings.Vinf, altitude,
//                         builder.getEngineData().getNumberOfEngines());
...
//                                            data.wingData, data.fuselageData,
//                                            data.engineData,
//                                            ac, wing, horizontal, vertical, fus, nac);
//     calcCenterOfGravity.getWeights();
//     calcCenterOfGravity.calculateCOGAircraft();

//     weightData    = calcCenterOfGravity.getWeightsData();
//     cogData       = calcCenterOfGravity.getCOGData();
//     cogComponents = calcCenterOfGravity.getCOGComponentsData();

  
//     // ── Pesi derivati ────────────────────────────────────────────────────────
//     double OEW = weightData.totalAircraftWeight
//                - weightData.payloadWeight
//                - fuelMass;

//     double structuralMass = weightData.wingWeight
//                           + weightData.horizontalWeight
//                           + weightData.verticalWeight
//                           + weightData.fuselageWeight
//                           + weightData.landingGearWeight
//                           + weightData.controlSurfacesWeight;

//     double manufacturerEmptyWeight = structuralMass
//                                    + weightData.propulsionGroupWeight
//                                    + weightData.avionicGroupWeight
//                                    + weightData.electricalGroupWeight
//                                    + weightData.hydraulicAndPneumaticWeight
//                                    + weightData.furnishingsAndEquipmentWeight
//                                    + weightData.airConditioningAndAntiIceWeight;

//     // ── Nuovo WTO ────────────────────────────────────────────────────────────
//     WTOnew = OEW + weightData.payloadWeight + fuelMass;
//     WTOhistory.push_back(WTOnew);
    

//     iteration++;
//     iterationHistory.push_back(iteration);
//     // il while confronta WTOnew appena calcolato vs WTOold del giro corrente ✓
// }


//   Plot plotConv(iterationHistory, WTOhistory,
//                           "Iteration (-)",
//                           "MTOM (kg)",
//                           "MTOM history",
//                           "MTOM convergence",
...
// WeightsAndBalanceExcelWriter excelWriterWeightsAndBalance;
// excelWriterWeightsAndBalance.writeWeightsDataAndBalanceToExcel(aircraftName + "_WeightsAndBalance.xlsx",
//                                                                aircraftName,
//                                                                settings,
//                                                                weightData,
//                                                                cogComponents);
